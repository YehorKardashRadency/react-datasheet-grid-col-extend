...
import { Cell, Column, ContextMenuItem, DataSheetGridProps, DataSheetGridRef, Operation, Selection } from '../types';
...
import { encodeHtml, parseTextHtmlData, parseTextPlainData } from '../utils/copyPasting';
...
const DEFAULT_DUPLICATE_ROW: DataSheetGridProps<any>['duplicateRow'] = ({ rowData }) => ({ ...rowData });
...
export const DataSheetGrid = React.memo(
  React.forwardRef<DataSheetGridRef, DataSheetGridProps<any>>(
    <T extends any>(
      ...
      const displayHeight = maxHeight ?? Math.min(400, headerRowHeight + totalSize(400) + heightDiff);
      ...
      const [expandingSelectionFromRowIndex, setExpandingSelectionFromRowIndex] = useState<number | null>(null);
      ...
      const expandSelection =
        ...
        (activeCell &&
          columns
            .slice((selection?.min.col ?? activeCell.col) + 1, (selection?.max.col ?? activeCell.col) + 2)
          ...
          if (innerBoundingClientRect && columnRights && columnWidths) {
            ...
            if (outerBoundingClientRect) {
              ...
              if (
                ...
                outerBoundingClientRect.right - event.clientX <= columnWidths[columnWidths.length - 1]
              ) {
      ...
      const duplicateRows = useCallback(
        ...
        [columns.length, duplicateRow, lockRows, onChange, setActiveCell, setSelectionCell, hasStickyRightColumn]
      );
          ...
          if (!editing && activeCell) {
            ...
            const textHtml = `<table>${copyData
              .map(
                (row) =>
                  `<tr>${row
                    .map((cell) => `<td>${encodeHtml(String(cell ?? '')).replace(/\n/g, '<br/>')}</td>`)
                    .join('')}</tr>`
              )
              .join('')}</table>`;
      ...
      const onMouseDown = useCallback(
        (event: MouseEvent) => {
          ...
          let cursorIndex = clickInside ? getCursorIndex(event, true, true) : null;
          ...
          if (event.target instanceof HTMLElement && event.target.className.includes('dsg-expand-rows-indicator')) {
            ...
            cursorIndex = getCursorIndex(event, true, true);
            setSelectionMode(
              cursorIndex && !rightClick
                ? {
                    columns: cursorIndex.col !== -1 || Boolean(event.shiftKey && activeCell),
          ...
          if ((!(event.shiftKey && activeCell) || rightClick) && data.length > 0) {
            setActiveCell(
              cursorIndex && {
                ...
                row:
                  (rightClickInSelection || rightClickOnSelectedGutter || clickOnSelectedStickyRightColumn) &&
                  activeCell
                    ? activeCell.row
                    : Math.max(0, cursorIndex.row),
                doNotScrollX: Boolean(
                  (rightClickInSelection && activeCell) || clickOnStickyRightColumn || cursorIndex.col === -1
                ),
          ...
          setSelectionMode(
            cursorIndex && !rightClick
              ? {
                  columns:
                    (cursorIndex.col !== -1 && !clickOnStickyRightColumn) || Boolean(event.shiftKey && activeCell),
                  rows: cursorIndex.row !== -1 || Boolean(event.shiftKey && activeCell),
                  active: true
                }
              : {
          ...
          if (event.shiftKey && activeCell && !rightClick) {
            ...
            if (cursorIndex && (cursorIndex?.col === -1 || cursorIndex?.row === -1 || clickOnStickyRightColumn)) {
      ...
      const onMouseUp = useCallback(() => {
        if (expandingSelectionFromRowIndex !== null) {
          if (activeCell && selection) {
            ...
            const min: Cell = activeCell;
            const max: Cell = activeCell;
            for (let row = min.row; row <= max.row; ++row) {
              copyData.push([]);

              for (let col = min.col; col <= max.col; ++col) {
                const { copyValue = () => null } = columns[col + 1];
                copyData[row - min.row].push(String(copyValue({ rowData: data[row], rowIndex: row }) ?? ''));
              }
            }
            Promise.all(
              ...
              const newData = [...data];
              for (let columnIndex = 0; columnIndex <= selection.max.col - selection.min.col; columnIndex++) {
                const pasteValue = columns[selection.min.col + columnIndex + 1]?.pasteValue;
                if (pasteValue) {
                  for (let rowIndex = selection?.min.row; rowIndex <= selection?.max.row; rowIndex++) {
                    if (
                      !isCellDisabled({
                        col: columnIndex + min.col,
                        row: rowIndex
            ...
            setSelectionCell({
              ...
              row: Math.max(activeCell?.row ?? 0, selection?.max.row ?? 0)
            });
          }
          ...
          if (expandingSelectionFromRowIndex !== null) {
            ...
            if (cursorIndex) {
              ...
              setSelectionCell(
                cursorIndex && {
                  col:
                    !copyByRow && activeCell
                      ? activeCell?.col
                      : Math.max(0, Math.min(lastColumnIndex, cursorIndex.col)),
                  row: copyByRow && activeCell ? activeCell.row : Math.max(0, cursorIndex.row),
      ...
      }, [selection, activeCell, deleteRows, duplicateRows, insertRowAfter, onCut, onCopy, applyPasteDataToDatasheet]);
      ...
      useImperativeHandle(ref, () => ({
        ...
        selection: getSelectionWithId(selection ?? (activeCell ? { min: activeCell, max: activeCell } : null), columns),
        setSelection: (value) => {
...
) as <T extends any>(props: DataSheetGridProps<T> & { ref?: React.ForwardedRef<DataSheetGridRef> }) => JSX.Element;
